...
/**
 * FENChess implements chess using a FEN string to initialize the board.
 * It uses the same bit–flag move–validation system as NaiveChess.
 */
export default class FENChess extends ChessInterface {
  /**
   * Constructs a FENChess instance.
   * @param {string} fen - A FEN string representing the board. Defaults to standard starting position.
   */
  ...
  /**
   * Parses a FEN string and initializes the board.
   * The board is represented as an 8×8 array of piece objects or null.
   * Piece objects are of the form: { type: string, color: "white" | "black" }.
   * @param {string} fen 
   */
  loadFEN(fen) {
    ...
    for (let row of rows) {
      ...
      for (let char of row) {
        if (/\d/.test(char)) {
          ...
          // Clone the object so that each piece is a separate object.
  ...
  // validateMove returns a bitmask (0 means VALID).
  validateMove(move) {
    let errors = 0;
    const { from, to } = move;
    ...
    if (!correctTurn) {
      ...
      // Only check destination occupancy if turn is correct.
    ...
    // Run piece-specific validations.
    switch (piece.type) {
      case "pawn":
