import NaiveChess from "../../src/NaiveChess/index.js";
...
function newBoard() {
  const b = new NaiveChess();
  b.currentTurn = "white";
  return b;
}

function runTests() {
  console.log("Starting 27-bitflag NaiveChess comprehensive tests...");
  ...
  // Expect: OUT_OF_BOUNDS (1) | NO_PIECE_AT_SOURCE (2) = 3.
  ...
  // Expect only OUT_OF_BOUNDS (1) since source is valid.
  ...
  result = board.validateMove(move);
  // Expect NO_PIECE_AT_SOURCE (2) only.
  ...
  board = newBoard();
  board.board[5][0] = { type: "pawn", color: "white" };
  move = { from: { row: 6, col: 0 }, to: { row: 5, col: 0 } };
  result = board.validateMove(move);
  // Turn is correct → check destination; since destination is friendly, add DESTINATION_OCCUPIED_BY_FRIENDLY (8)
  // plus pawn validation (ILLEGAL_PAWN_MOVE, 16) = 8|16 = 24.
  ...
  board = newBoard();
  board.board[7][0] = null; // clear destination
  ...
  board = newBoard();
  board.board[5][3] = { type: "pawn", color: "black" };
  ...
  board = newBoard();
  board.board[7][3] = null; // clear destination
  ...
  board = newBoard();
  board.board[6][3] = null; // clear path
  ...
  board = newBoard();
  board.board[6][2] = null; // clear destination
  ...
  board = newBoard();
  board.board[7][3] = null; // clear destination so that error is due to blocked path.
  move = { from: { row: 7, col: 0 }, to: { row: 7, col: 3 } };
  result = board.validateMove(move);
  // Expected: PATH_NOT_CLEAR (1024) because the path is blocked by pieces at (7,1) and (7,2).
  ...
  board = newBoard();
  board.board[6][4] = null; // clear destination (normally a pawn)
  ...
  board = newBoard();
  board.currentTurn = "black";
  // Clear destination (7,3) so destination occupancy is not added.
  board.board[7][3] = null;
  // White knight at (7,1) moving invalidly: (7,1)->(7,3) is not L-shaped.
  move = { from: { row: 7, col: 1 }, to: { row: 7, col: 3 } };
  result = board.validateMove(move);
  ...
  board = newBoard();
  board.board[5][1] = { type: "pawn", color: "white" };
  ...
  result = board.validateMove(move);
  // Source out-of-bounds → return OUT_OF_BOUNDS|NO_PIECE_AT_SOURCE = 1|2 = 3.
  ...
  assertEqual(board.board[6][0], null, "Test 26 failed: Pawn source square not empty after execution", board, move);
  ...
  board.movePiece(move);
  const finalConfig = board.getGameState();
  assertEqual(finalConfig[7][1], null, "Test 27 failed: Knight source square not empty after move", board, move);
  if (!finalConfig[5][0]) {
  ...
  console.log("All 27 NaiveChess tests passed successfully.");
}
